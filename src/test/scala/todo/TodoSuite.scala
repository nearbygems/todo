...
class TodoSuite extends FunSuite :
  ...
  def allTests(fixture: FunFixture[(String, Model)]): Unit =
    ...
    fixture.test("complete changes state to completed, if task is not already completed"){ case (name, model) =>
      ...
      assertEquals(read1, returned1,
                   s"Using $name: Reading the task $id1 returned a different value from completing it")
      assertEquals(read2, returned2,
                   s"Using $name: Reading the task $id2 returned a different value from completing it")
    }
